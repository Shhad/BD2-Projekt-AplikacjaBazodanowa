...
public class SeasonMatchListener implements ActionListener{
	...
	class MatchListListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent event) {
			if(!event.getValueIsAdjusting()) {
				option =  (MatchInSeason) SeasonMatchListener.listMatch.getSelectedValue();
				for(int i = 0;i<MainFrame.data.getTeams().size();i++) {
					if(MainFrame.data.getTeams().get(i).getName() == option.getTeam1()) {
						team1 = MainFrame.data.getTeams().get(i);
					}
					if(MainFrame.data.getTeams().get(i).getName() == option.getTeam2()) {
						team2 = MainFrame.data.getTeams().get(i);
					}
				}
	...
	class SeasonListListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent event) {
			if(!event.getValueIsAdjusting()) {
				...
				for(int i = 0;i<MainFrame.data.getMatches().size();i++) {
					if(MainFrame.data.getMatches().get(i).getSeasonID() == option.getId()) {
						matchesS.add(MainFrame.data.getMatches().get(i));
					}
				}
	...
	public class MatchListCellRenderer extends DefaultListCellRenderer
	{
	        ...
	        JList list, Object value, int index, boolean isSelected, boolean cellHasFocus)
	    {
	       ...
	       for(int i = 0;i<MainFrame.data.getTeams().size();i++) {
				if(MainFrame.data.getTeams().get(i).getName() == help.getTeam1()) {
					team1 = MainFrame.data.getTeams().get(i);
				}
				if(MainFrame.data.getTeams().get(i).getName() == help.getTeam2()) {
					team2 = MainFrame.data.getTeams().get(i);
				}
			}
